...
export async function createSpritesheetPet(container, petData) {
  ...
  const displayWidth = config.width || 150;
  const displayHeight = config.height || 150;
  const scale = displayHeight / frameHeight;
  const scaledFrameWidth = frameWidth * scale;
  const scaledFrameHeight = frameHeight * scale;
  const scaledSheetWidth = spriteConfig.columns * frameWidth * scale;
  const scaledSheetHeight = spriteConfig.rows * frameHeight * scale;

  const petEl = document.createElement('div');
  petEl.className = 'pet spritesheet-pet idle';
  petEl.style.width = `${displayWidth}px`;
  petEl.style.height = `${displayHeight}px`;
  petEl.style.position = 'relative';
  petEl.style.overflow = 'hidden';

  const spriteEl = document.createElement('div');
  spriteEl.style.position = 'absolute';
  spriteEl.style.width = `${scaledFrameWidth}px`;
  spriteEl.style.height = `${scaledFrameHeight}px`;
  spriteEl.style.backgroundImage = `url(${imgUrl})`;
  spriteEl.style.backgroundRepeat = 'no-repeat';
  spriteEl.style.backgroundSize = `${scaledSheetWidth}px ${scaledSheetHeight}px`;
  spriteEl.style.left = `${(displayWidth - scaledFrameWidth) / 2}px`;
